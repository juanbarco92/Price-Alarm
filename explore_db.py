...
def explore_database(db_path: str = "db/prices.db"):
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        for i, (name, url, count, min_price, max_price, avg_price, first_date, last_date) in enumerate(products, 1):
            print(f"\n{i}. {name}")
            print(f"   📍 URL: {url[:60]}{'...' if len(url) > 60 else ''}")
            print(f"   📊 Registros: {count}")
            print(f"   💰 Precio mín: ${min_price:,.0f}")
            print(f"   💰 Precio máx: ${max_price:,.0f}")
        ...
        for name, price, old_price, timestamp in recent_records:
            dt = datetime.fromisoformat(timestamp)
            old_price_str = f" (antes: ${old_price:,.0f})" if old_price else ""
            print(f"• {dt.strftime('%Y-%m-%d %H:%M')} - {name}: ${price:,.0f}{old_price_str}")
